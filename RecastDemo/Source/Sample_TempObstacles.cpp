...
void drawDetailOverlay(const dtTileCache* tc, const int tx, const int ty, double* proj, double* model, int* view)
{
	...
	for (int i = 0; i < ntiles; ++i)
	{
					   ...
					   model, proj, view, &x, &y, &z))
		{
			...
			snprintf(text, 128, "pos: (%f, %f, %f) ", pos[0], pos[1], pos[2]);
			imguiDrawText((int)x, (int)y - 85, IMGUI_ALIGN_CENTER, text, imguiRGBA(0, 0, 0, 128));
		}
	}
}
...
public:
	...
	virtual void handleMenu()
	{
		...
		if (imguiCheck("Draw Postion", m_drawType == DRAWDETAIL_POSITION))
			m_drawType = DRAWDETAIL_POSITION;
	}
...
void Sample_TempObstacles::handleSettings()
{
	...
	if (imguiButton("Save"))
	{
		...
		if (!m_geom || !m_geom->getMesh())
			meshname = "all_tiles_tilecache.bin";
		else
		{
			std::string filepath = m_geom->getMesh()->getFileName();
			size_t extensionPos = filepath.find_last_of('.');
			size_t folderPos = filepath.find_last_of('/');
			if (extensionPos == std::string::npos || folderPos == std::string::npos)
				return;
			int startpos = folderPos + 1;
			int count = extensionPos - startpos;
			meshname = filepath.substr(startpos, count);
			meshname = "bins/" + meshname + ".bin";
		}

		saveAll(meshname.c_str());
	}
	...
	if (imguiButton("Load"))
	{
		...
		dtFreeNavMesh(m_navMesh);
		dtFreeTileCache(m_tileCache);
		loadAll(meshname.c_str());
		m_navQuery->init(m_navMesh, 2048);
	}

	if (imguiButton("Save TempObstacles"))
	{
		//saveObstaclesAll("All_TempObstaclesCache.bin");
		saveObstacles("All_TempObstaclesCache.bin");
		//saveAll("All_TempObstaclesCache.bin");
	}

	if (imguiButton("Load TempObstacles"))
	{
		//loadObstaclesAll("All_TempObstaclesCache.bin");
		loadObstacles("All_TempObstaclesCache.bin");
	}
...
void Sample_TempObstacles::loadObstaclesAll(const char* path)
{
	FILE* fp = fopen(path, "rb");
	if (!fp) return;
	int count = 0;
	size_t headerReadReturnCode = fread(&count, sizeof(int), 1, fp);
	if (headerReadReturnCode != 1)
	{
		fclose(fp);
		return;
	...
	for (int i = 0; i < count; ++i)
	{
		float bmin[3];
		float bmax[3];
		size_t tileHeaderReadReturnCode = fread(bmin, sizeof(float), 3, fp);
		tileHeaderReadReturnCode = fread(bmax, sizeof(float), 3, fp);

		dtObstacleRef refid;
		tileHeaderReadReturnCode = fread(&refid, sizeof(dtObstacleRef), 1, fp);
		m_tileCache->addBoxObstacle(bmin, bmax, &refid);
	}
	fclose(fp);
};

void Sample_TempObstacles::saveObstaclesAll(const char* path)
{
	if (!m_tileCache) return;
	...
	for (int i = 0; i < m_tileCache->getObstacleCount(); ++i)
	{
		...
		dtObstacleRef refid = m_tileCache->getObstacleRef(ob);

		//float pos[3];
		//pos[0] = (bmin[0] + bmax[0]) / 2.0f;
		//pos[1] = bmin[1];
		//pos[2] = (bmin[2] + bmax[2]) / 2.0f;
		fwrite(bmin, sizeof(float), 3, fp);
		fwrite(bmax, sizeof(float), 3, fp);
		fwrite(&refid, sizeof(dtObstacleRef), 1, fp);
	}
	fclose(fp);
};
...
void Sample_TempObstacles::loadObstacles(const std::string& path) const
{
	if (!m_tileCache) return;
...
void Sample_TempObstacles::saveObstacles(const std::string& path)
{
	if (!m_tileCache) return;
