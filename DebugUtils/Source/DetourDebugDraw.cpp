	...
	for (int i = 0; i < tile->header->polyCount; ++i)
	{
		...
		else
		{
			if (flags & DU_DRAWNAVMESH_COLOR_TILES)
				col = duRGBA(0, 255, 0, 255);
			else
				col = duRGBA(255, 0, 0, 255);//duTransCol(dd->areaToCol(p->getArea()), 64);
		}

// 		for (int j= 0; j < p->vertCount; ++j)
// 		{
// 			unsigned short vertIndex = p->verts[j];
// 			float* pos = &tile->verts[vertIndex];
// 			dd->vertex(pos, col);
// 		}
		
		for (int j = 0; j < pd->triCount; ++j) //获取当前poly的组成的三角形数量
		{
...
void duDebugDrawTileCacheClickPos(duDebugDraw* dd, const struct dtTileCachePolyMesh& lmesh,
								  const float* orig, const float cs, const float ch)
{
